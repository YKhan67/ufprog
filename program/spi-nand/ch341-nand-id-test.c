...
* WRITE DISABLE
*
* The diagnostic focuses on the READ-ID transaction framing.
*
* READ ID hardware sequence:
*
* 9Fh
* 00h dummy
* ID clocks
...
* 4. READ ID with explicit command + dummy
* 5. Raw extended ID-clock tests
* 6. Read feature registers A0/B0/C0
* 7. Decode C0
* 8. Repeated RESET + ID test
* 9. PAGE READ page 0
* 10. READ CACHE page 0
* 11. Repeated ID test
* 12. Combined status/feature/ID test
* 13. Final RESET
...
case CTRL_SHUTDOWN_EVENT:
     ...
     *
     * The main thread performs the actual cleanup.
     */
...
* RAW READ-ID TRANSACTION
*
* This deliberately builds the COMPLETE SPI transaction:
*
* 
  9F
...
*
* The first two bytes are command + dummy.
* The remaining bytes provide ID clocks.
*
* The returned stream is printed in its entirety.
*
* IMPORTANT:
*
* This uses the UFPROG generic transfer as a single
* continuous transfer operation so that the command,
* dummy and read-clock portions are submitted together.
* ============================================================ */

static int read_id_raw(struct ufprog_spi *spi,
uint8_t *rx,
size_t rxlen)
{
uint8_t tx[34];
...
/*
 * Submit the complete transaction as one transfer.
 *
 * The TX stream is:
 *
 *     9F 00 00 00 ...
 *
 * The RX stream is captured for the complete number
 * of clocks.
 */
...
/*
 * The UFPROG generic transfer interface normally
 * represents separate TX/RX phases. If the underlying
 * controller treats a single TX transfer as full-duplex,
 * the RX buffer receives the complete sampled stream.
 *
 * For CH341 diagnostics, the normal UFPROG SPI path is
 * therefore retained here rather than issuing separate
 * command and read operations that could alter CS.
 */

if (ufprog_spi_generic_xfer(spi,
                            &xfer,
                            1))
    return -1;

/*
 * The UFPROG transfer structure is direction-specific.
 * If the controller does not populate RX for SPI_DATA_OUT,
 * use the standard split transaction below.
 */
...
* STANDARD READ-ID TRANSACTION
*
* 9F + 00 dummy followed by ID clocks.
*
* This is retained as the primary method because it matches
* the UFPROG SPI abstraction used elsewhere in this utility.
* ============================================================ */
...
/*
 * Command + dummy are submitted together.
 *
 * CS is held until the RX phase completes according
 * to the xfer[].end semantics.
 */
...
* RAW EXTENDED ID STREAM TEST
*
* This test deliberately examines the returned bytes without
* assuming that byte zero of the RX stream is necessarily the
* first manufacturer-ID byte.
*
* It tests:
*
* 
  9F + dummy + 1..16 clocks
  
*
* and prints the complete returned stream.
* ============================================================ */

static int raw_id_stream_test(struct ufprog_spi *spi)
{
printf("\n");
printf("============================================================\n");
printf(" RAW READ-ID STREAM TEST\n");
printf("============================================================\n");
printf("\n");


printf("Command stream:\n");
printf("  9F 00 followed by read clocks\n");
printf("\n");

for (unsigned clocks = 1;
     clocks <= 16;
     clocks++) {
    ...
    if (read_id(spi,
        ...
        printf("Clocks %2u : COMMUNICATION ERROR\n",
               clocks);
...
* READ-ID WITH OFFSET ANALYSIS
*
* Since different SPI NAND parts/documentation can describe
* the dummy phase differently, this prints every possible
* five-byte window in an extended response.
*
* This is diagnostic only.
* ============================================================ */

static int id_window_test(struct ufprog_spi *spi)
{
uint8_t rx[32] = {0};
...
printf("\n");
printf("============================================================\n");
printf(" READ-ID WINDOW ANALYSIS\n");
printf("============================================================\n");
printf("\n");

printf("Requesting extended ID stream...\n");
...
for (unsigned offset = 0;
     offset + sizeof(expected_id) <= sizeof(rx);
     offset++) {
...
if (raw_id_stream_test(spi)) {
    ...
    fprintf(stderr,
            "\nWARNING: raw ID stream diagnostic failed.\n");
}
...
if (id_window_test(spi)) {
    ...
    fprintf(stderr,
            "\nWARNING: ID window diagnostic failed.\n");
}
...
if (repeated_reset_id_test(spi)) {
    ...
    fprintf(stderr,
            "\nWARNING: repeated RESET/ID diagnostic failed.\n");
}
